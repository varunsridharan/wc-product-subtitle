...
class WooCommerce_Product_Subtitle_Settings_Framework {
    private $page_hook = '';
    public $settings;
    ...
    private function add_settings_pages(){
		$pages = array();
		$pages = apply_filters('wc_ps_settings_pages',$pages);
    ...
    private function add_settings_fields(){
        global $fields;
        $fields =  array();
		$fields = apply_filters('wc_ps_settings_fields',$fields);
                ...
                while($a < $c){
                    if(isset($s[$a]['validate_callback'])){
                        ...
                        $file = addslashes(PUGIN_SETTINGS.'validate-'.$s[$a]['id'].'.php');
                         $s[$a]['validate_callback'] = create_function('$fields', 'do_action("wc_ps_settings_validate",$fields); do_action("wc_ps_settings_validate_'.$s[$a]['id'].'",$fields);');
                    }
                    $a++;
                }
            }
    ...
    function admin_init(){ 
		$this->settings = new WooCommerce_Product_Subtitle_WP_Settings();
		...
		$this->settings->init($pages, WCPS_DB );
    }
    
    
    public function admin_page(){
		echo '<div class="wrap wc_ps_settings">';
		settings_errors();
    ...
    function get_settings($key = ''){
        ...
        foreach($this->settings_page as $settings){
            $this->settings_key[] = WCPS_DB.$settings['slug'];
            $db_val = get_option(WCPS_DB.$settings['slug']);
            if(is_array($db_val)){
